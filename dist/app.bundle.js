...
var _insertData = __webpack_require__(34);

var _insertData2 = _interopRequireDefault(_insertData);

var _uniq = __webpack_require__(35);

var _uniq2 = _interopRequireDefault(_uniq);

var _verifyProxy = __webpack_require__(36);

var _verifyProxy2 = _interopRequireDefault(_verifyProxy);
...
(0, _verifyProxy2.default)('proxy-list');
...
function insertData(collectionName, arrProxy) {
    return db.find().where('name').eq(collectionName).exec(function (err, list) {
        ...
        if (err) {
            ...
            db.update({ name: collectionName }, { $set: { urls: newlist } }, { safe: true, upsert: true }, function (err, rawResponse) {
                if (err) {
                    console.log(err);
                } else {
...
function checkrep(collectionName) {
    db.find().where('name').eq(collectionName).exec(function (err, list) {
        if (err) {
            console.log(err);
        } else {
    ...
    return new _promise2.default(function (resolve, reject) {
        ...
        request(targetOptions, function (error, response, body) {
            try {
                ...
                body = body.toString();
                var arr = body.split("'");
                var ret = { ip: arr[1], address: arr[3] };
